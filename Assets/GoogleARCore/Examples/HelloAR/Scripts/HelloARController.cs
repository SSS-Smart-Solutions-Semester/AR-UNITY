...
namespace GoogleARCore.Examples.HelloAR
{
    ...
    public class HelloARController : MonoBehaviour
    {
        ...
        public void Update()
        {
            ...
            if (!Frame.Raycast(touch.position.x, touch.position.y, raycastFilter, out hit)) return;
            if (currentNumberOfObjects >= numberOfObjectAllowed) return;
            currentNumberOfObjects += 1;
            var catObject = Instantiate(GameObjectPointPrefab, hit.Pose.position, hit.Pose.rotation);
            var anchor = hit.Trackable.CreateAnchor(hit.Pose);

            if ((hit.Flags & TrackableHitFlags.PlaneWithinPolygon) != TrackableHitFlags.None)
            {
                var cameraPositionSameY = FirstPersonCamera.transform.position;
